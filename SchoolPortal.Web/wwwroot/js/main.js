(function ($) {
    ...
    /*-------------------------------------
      ...
      -------------------------------------*/
    $(".sidebar-toggle-mobile, .overlay").on("click", function () {
    ...
    //=================== Custom =================
    $('body').on('mouseenter', '.sidebar-collapsed .sidebar-nav-item .nav-link', (e) => {
        let item = $(e.currentTarget);
        let offset = item.offset().top - $(window).scrollTop();
        item.next().css('position', 'fixed').css('top', offset);
    });
    // .on('mouseleave', '.sidebar-collapsed .sidebar-nav-item .nav-link', (e)=>{
    //   let item = $(e.currentTarget);
    //   let offset = item.offset().top - $(window).scrollTop();
    //   item.next().css('position', 'absolute').css('top', 0);
    // });
    $(window).resize(() => {
        if ($(window).width() < 768) {
            $('#wrapper').removeClass('sidebar-collapsed');
...
function removeSubmenuFixed() {
    $menus = $('.sidebar-menu-one .sidebar-menu-content .nav-sidebar-menu > .nav-item .sub-group-menu');
    $menus.css('position', '').css('top', '');
}